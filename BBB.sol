//https://t.me/BabyBoneShibar
//https://x.com/babyboneshibar
...
contract BABYBONE is IBEP20, Auth {
    
    using SafeMath for uint256;

    string constant _name = "BabyBone";
    string constant _symbol = "BBB";
    uint8 constant _decimals = 18;

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address routerAddress = 0x52bfe8fE06c8197a8e3dCcE57cE012e13a7315EB;
    ...
    uint256 public BONERewardsFee = 60;
    uint256 public developmentFee    = 0; 
    uint256 public liquidityFee  = 10;
    uint256 public totalFees     = BONERewardsFee + developmentFee + liquidityFee;
    ...
    function setFeeRates(uint256 _BONERewardsFee, uint256 _liquidityFee, uint256 _developmentFee, uint256 _feeDenominator) public onlyOwner {
            BONERewardsFee = _BONERewardsFee;
            liquidityFee = _liquidityFee;
            developmentFee = _developmentFee;
            totalFees = _BONERewardsFee + _liquidityFee + _developmentFee;
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 amountBONE = address(this).balance;

        uint256 totalBONEFee = totalFees.sub(liquidityFee.div(2));
        
        uint256 amountBONELiquidity = amountBONE.mul(liquidityFee).div(totalBONEFee).div(2);
        uint256 amountBONEReflection = amountBONE.mul(BONERewardsFee).div(totalBONEFee);
        uint256 amountBONETeam = amountBONE.sub(amountBONELiquidity).sub(amountBONEReflection);

        try dividendDistributor.deposit{value: amountBONEReflection}() {} catch {}
                
        if(developmentFee > 0){
            payable(devWallet).transfer(amountBONETeam);
        }

        if(amountToLiquify > 0){
            router.addLiquidityETH{value: amountBONELiquidity}(
                address(this),
                amountToLiquify,
                0,
                0,
            ...
            emit AutoLiquify(amountBONELiquidity, amountToLiquify);
        }
    }
